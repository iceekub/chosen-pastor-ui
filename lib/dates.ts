...
/**
 * Compact relative time for dense list rows, e.g. "just now", "4m ago",
 * "3h ago", "2d ago". Falls back to a short absolute date beyond 7 days
 * so very old rows stay scannable. Pair with `title={timestamp}` (and
 * `suppressHydrationWarning` when rendered during SSR) since the value
