 ...
 *
 * Runs on every protected route. Decodes the Supabase access token stored
 * in the session cookie and — if it's within REFRESH_THRESHOLD seconds of
 * expiry — exchanges the refresh token for a new pair and rewrites the
 * cookie before the request hits any page or API route.
 *
...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // Invalid / expired session cookie — let verifySession handle the redirect.
